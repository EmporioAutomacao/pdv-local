...
| Coleta so pega inclusao, nunca alteracao (preco/saldo mudam no Arpa e nao chegam ao ERP) | Views `sync_export` com `occurred_at_utc` fixo (carga inicial) - o watermark passou dele e nada novo aparece. | 1.6.9: rodar **Preparar views** de novo — o script novo usa as tabelas de log (`alterados`/`produtos_altera_quantidade`) como fonte temporal e sincroniza incremental. Para reenviar o que ficou pra tras: botao **Sincronizar tudo**. |
| Falha em `sync_export.estoque`/`vendas`/`financeiro` no log de sincronizacao | Toggle marcado mas a view nao existe nesse Arpa (ex.: sem tabela de pedidos). | Desmarcar o toggle na conexao. Estoque: **Preparar views** cria a view se `produtos` tiver coluna de quantidade. |
